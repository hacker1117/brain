---
type: original
title: 李维斯第一个培训项目合同——GWC里程碑
date: '2026-05-15T00:00:00.000Z'
tags:
  - GWC
  - 合同
  - 培训
  - 李维斯
  - 里程碑
---

# 李维斯第一个培训项目合同——GWC里程碑

Henry 在确认 GWC 培训合同最终版时说：

> "确认这是最终版了，可以保存进 gBrain 了，这是我的公司的第一个培训项目合同哈，记得这些关联关系也要存好"

## 背景

- 客户：[[entities/companies/gwc|GWC]]
- 项目：2026-05-17 一整天 AI 培训（苏州吴中区繁丰路666号）
- 意义：这是[[entities/companies/livis|李维斯智能科技工作室]]作为 OPC 开展培训业务以来签下的**第一份正式合同**
- 合同文件：李维斯-GWC-培训服务合同-20260517

## 合同要点

- 付款信息：使用公司公户
- 培训地址：江苏省苏州市吴中区繁丰路666号
- 格式：标准合同文本格式（无表格），甲乙方信息以条款形式呈现

[Source: User, Telegram/OpenClaw Transcript, 2026-05-15]
